...
from lunaengine.misc.icons import Icons  # Import the icons module
...
class ComprehensiveUIDemo(Scene):
    ...
    def setup_icons_tab(self):
        ...
        self.icon_size_selector.set_on_value_changed(self.update_icons_size)
        ...
        # Get all icons
        self.all_icons = Icons.get_all(32)
        self.icon_elements = []
    ...
    def update_icons_display(self):
        ...
        self.all_icons = Icons.get_all_dict(icon_size)
        
        # Grid layout
        spacing = 20
        icon_with_spacing = icon_size + spacing + (30 * (icon_size / 32))  # Extra space for label
        icons_per_row = int(self.icons_scroll.width // icon_with_spacing)
        
        x_offset = 20
        y_offset = 20
        
        icons_color = ColorKeys([(random.randint(0,255), random.randint(0,255), random.randint(0,255), random.uniform(.7, 1.0)) for _ in range(5)])
        
        # Add each icon with label
        for i, (icon_name, icon_object) in enumerate(self.all_icons.items()):
            row = i // icons_per_row
            col = i % icons_per_row
            
            x = x_offset + col * icon_with_spacing
            y = y_offset + row * (icon_size + 30 * (icon_size / 32))  # Extra space for label
            ...
            icon_img.set_image(icon_object.get_surface(icons_color))
    ...
    def toggle_icons_background(self, show: bool):
        ...
        for element in self.icon_elements:
            if isinstance(element, UiFrame):
                if show:
                    element.set_background_color(ThemeManager.get_color('background'))
                    element.set_border(ThemeManager.get_color('border'), 1)
                else:
