...
- Scenes, the scenes control is based on Godot, so, it probably will be useful for like, 80% of the users;
- Particles, i proudly announce that we have one of the most optmized particles systems in pygame(*maybe*);
- Finally, Cameras are very very headache for work when you are making a demo game for example, so i simplified this here.
