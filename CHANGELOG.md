...
<small>This update is the last to 0.2.6!</small>
...
- **Refactor:** Complete rewrite of the mouse input system around **Pointer Capture**;
  - **Change:** ``InputState`` tracks a single ``captured_element`` per press — all mouse events route to it until release;
  ...
  - **Added:** ``is_interactive`` class attribute on ``UIElement`` — plain containers are no longer capture targets; region-based interactivity for ``UiFrame``(header), ``ScrollingFrame``(thumbs) and ``Tabination``(tab area);
  ...
  - **Removed:** Local drag flags: ``UiFrame._dragging``, ``ScrollingFrame.dragging_vertical``/``dragging_horizontal``, ``Slider.dragging``, ``TextArea._dragging``, ``Dropdown.is_scrolling``;
  - **Fixes:** Click-through on popups; elements inside ``UiFrame``/``ScrollingFrame``/``Tabination``/expanded ``ColorPicker`` not receiving clicks; ``ScrollingFrame`` double child update; ``Dropdown``/``ColorPicker`` one-frame layer lag on expand; drag conflicts between frames, sliders, scrollbar thumbs and text selection;
- **New System:** Console **REPL** on ``LiveInspector``:
  - **Feature:** Multi-line command bar — ``Enter`` inserts a newline, ``Ctrl+Enter`` executes; visible ``Run`` button executes same as ``Ctrl+Enter``;
  - **Feature:** Preloaded namespace, no ``import`` required: ``engine``, ``scene``, ``ui``, ``sel``, ``LayerType``, all public ``UIElement`` subclasses, ``json``, ``os``, ``sys``, ``math``, ``random``, ``time``, ``pathlib.Path``; custom functions registered in ``LiveInspector`` callable by name;
  - **Feature:** ``import`` blocked through a restricted ``__builtins__`` dictionary — no ``__import__``;
  - **Feature:** Expressions print via ``repr``, statements execute silently, exceptions log at ``ERROR`` and never crash; existing ``stdout``/``stderr`` redirect preserved;
  - **Fixes:** Double character per key press(insertion on ``KEYDOWN`` only; ``TEXTINPUT``/printable ``KEYUP`` suppressed); unbounded key repeat(now uses Timer cooldowns, no ``time.sleep``);
- **New System:** **Hot Reload** for scenes and modules:
  - **Feature:** ``engine.hot_reload: bool = False`` constructor kwarg, togglable from the LiveInspector Settings tab;
  - **Feature:** ``engine.watch_scene(scene_ref)`` / ``engine.watch_module(ref)`` — ref accepts a module, import name, ``str``/``Path`` source file, or path-backed ``AtlasItem``(packed ``.res`` items raise ``ValueError``);
  - **Feature:** ``engine.on_scene_reload(name, callback)`` / ``engine.on_module_reload(name, callback)``;
  - **Feature:** Polling driven by the existing ``Timer`` system(no threads, no ``time.sleep``) — reload fires only when mtime changes, with one stable poll cycle of debounce;
  - **Feature:** Scene reload rebuilds via ``__init__``(``on_exit`` → new instance → ``on_enter``) — no state preservation; ``SyntaxError`` during reload logs to the console pane and keeps the previous scene/module running;
  - **Docs:** ``from module import X`` bindings are snapshots and do not update — use ``import module; module.X``; watched modules do not cascade;
- **New System:** Cursor images:
  - **Feature:** ``lunaengine/core/cursor.py`` — ``CursorManager`` with ``set_state(name) -> bool``, exposed as ``engine.cursor_manager`` and ``engine.set_cursor_state(name)``;
  - **Feature:** Preset names ``arrow``, ``hand``, ``ibeam``, ``crosshair``, ``resize_h``, ``resize_v``, ``sizeall``, ``wait``, ``no``; also accepts ``pygame.Cursor``, ``pygame.Surface``, ``AtlasItem``, ``Icon``, ``Path``, string path, and ``graphics.Image``;
  - **Feature:** ``UIElement`` gained additive ``default_cursor``, ``hover_cursor`` and ``pressed_cursor`` class attributes — resolved each frame from the topmost hit element with pressed-over-hover-over-default priority;
- **New System:** ``lunaengine/utils/clipboard.py`` — ``copy_text(text) -> bool`` and ``paste_text() -> str``, backed by ``pygame.scrap``(X11 and Windows, no new dependencies); handles empty clipboard, bytes/str decoding, and failure paths;
- **Feature:** ``TextBox`` and ``TextArea`` support ``Ctrl+C``/``Ctrl+V``/``Ctrl+X``; ``TextBox`` also supports ``Ctrl+A`` and ``Shift``-based selection; console REPL command bar inherits the same shortcuts without changing its ``Ctrl+Enter`` execute behavior;
- **Added:** ``lunaengine/utils/url.py``:
  - **Feature:** ``open_url(url) -> bool`` — validates ``http``/``https``/``mailto`` and delegates to stdlib ``webbrowser``; rejects malformed URLs;
  - **Feature:** ``fetch_url(url, timeout=5.0, parse='auto')`` — returns ``bytes``/``str``/``dict``/``None``; supports ``bytes``/``text``/``json``/``auto``; accepts only ``http``/``https``; non-2xx and all failures return ``None``; blocking(docstring warns against calling on the main loop for slow endpoints);
- **New System:** ``ui/modals.py`` — shared ``Overlay`` base class(``UIElement`` subclass) with ``show``/``hide``/``set_position``/``cleanup_on_hide``/``update_overlay_fade`` and an ``opacity`` property; ``Tooltip``, ``Notification`` and ``DialogBox`` now inherit from it;
...
- **Added:** ``Scene.on_enter`` now accepts ``args`` and ``kwargs`` passed through ``set_scene``;
- **Enhancement:** ``Timer`` gained ``Timer.now()``, ``Timer.ready(name, duration)`` and ``Timer.clear_ready(name)`` — non-blocking cooldown checks for game code; ``Timer.wait_for_end()`` no longer blocks with ``time.sleep``;
- **Enhancement:** ``LiveInspector`` improvements:
  - Registers as a real UI-layer element with a ``_debug_only`` flag — fixes click-through to game UI and gives REPL/scrollbars/property lists proper scroll and keyboard input;
  - Selection highlight(solid 1px outline on selected element's screen bounds using the ``ThemeManager`` accent color; dimmer outline on tree hover);
  - Icons in the Elements hierarchy(mapped by class name with a ``cube`` fallback);
  - Titlebar with a ``debug`` icon on the left and Windows/Linux-style Minimize/Hide controls on the right — Minimize collapses the body to the header and skips both update and render;
  - Text uses the engine's default font from ``FontManager``/atlas instead of the OS default;
- **Enhancement:** Container culling extended to ``UiFrame``-derived containers via an opt-in ``cull_children`` property(default ``False``); ``ScrollingFrame`` retains its existing ``cull_updates``/``cull_rendering`` controls;
  - Shared ``_container_clip_rect`` and ``_child_intersects_container`` helpers — read-only, live(never cached), ancestor-aware(intersects against every parent clip rect);
  - **Fixes:** Culling no longer mutates state during render(removed ``child.x``/``child.y`` writes in ``ScrollingFrame.render`` and ``UiFrame.render`` that caused icon/text offset drift); uses intersection not containment; focused, dragged and actively animated children are never culled; hit-test, keyboard focus, pointer capture, layout and ``TextBox`` editing unaffected;
- **Refactor:** ``Expandable`` fully rewritten as ``Accordion``(W3Schools accordion pattern) — single-open-at-a-time by default, optional ``allow_multiple``, non-blocking height animation, optional ``animation_speed`` parameter;
- **Refactor:** File consolidation:
  - ``ui/styles.py`` deleted; ``UIState`` moved into ``ui/elements/base.py``;
  - ``ui/elements/clock.py`` and ``ui/elements/progress.py`` merged into ``ui/elements/misc.py``;
  - ``ui/elements/dialogs.py`` merged into ``ui/elements/containers.py``;
  - ``ui/elements/visualizers.py`` merged into ``ui/elements/misc.py``(``ChartVisualizer``, ``AudioVisualizer``, ``Table``);
  - ``ui/tooltips.py`` and ``ui/notifications.py`` merged into new ``ui/modals.py``;
  - ``DialogBox`` moved from ``ui/elements/containers.py`` into ``ui/modals.py``;
- **Fixes:** ``ui/modals.py`` imports ``_get_ratio_scale`` and ``_scaled_shadow`` from ``ui.elements``; re-exported from ``ui/elements/__init__.py``;
- **Fixes:** ``ui/elements/misc.py`` now explicitly imports ``pathlib.Path``;
...
- **Fixes:** ``Dropdown`` searchable:
  - Clicking the search box no longer closes the dropdown — close check now uses the arranged screen rect from ``getCollideRect()`` and unions it with the expanded area;
  - Keyboard dispatcher in ``scene.py`` raised ``AttributeError`` on elements without a ``focused`` attribute(notably ``Dropdown``), aborting the key event loop and breaking all ``TextBox``/``TextArea`` input — reads now use ``getattr`` guards in both ``KEYDOWN`` and ``KEYUP`` paths;
- **Fixes:** Removed a leftover debug ``print(args, kwargs)`` in ``ui_comprehensive_demo.py`` that leaked tuple/dictionary output during startup;
- **Added:** Icon system now accepts ``float`` sizes;
- **UI:** Reworked icon art with thicker strokes for better readability at small sizes;
- **Fixes:** ``TextArea`` handles floating/partial lines correctly; ``TextBox`` no longer breaks when ``None`` is passed as text;
- **Examples:** ``ui_comprehensive_demo.py`` gained a **Utils** tab:
  - URL section(``fetch_url`` with parsed result and error display, ``open_url`` button, status label showing ``fetched``/``opened``/``offline``);
  - Clipboard section(sample ``TextBox`` with Copy, Paste filling a ``TextArea``, Copy Debug Info with version/FPS/scene, status label);
  - Cursor scenarios(hand, ibeam, draggable ``sizeall``);
  - Demo stays runnable offline — ``fetch_url`` returning ``None`` shows ``offline`` instead of raising;
- **Examples:** Added ``examples/hot-reload/hot_reload_demo.py``(two scenes + watched ``stats.py`` module), ``examples/hot-reload/hot_reload_scenes.py`` and ``examples/hot-reload/stats.py``; documented in ``examples/examples.md``;
- **Docs:** ``examples/timer_demo.py`` now documents the non-blocking ``engine.timer.ready(name, seconds)`` cooldown API;
